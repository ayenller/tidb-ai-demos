...
const I18N = {
  ...
  en: {
    ...
    embedMs: (n) => `Embedding the query took ${n} ms — a round trip to the model provider, not to TiDB. The per-column timings are the SQL alone.`,
  ...
  zh: {
    ...
    embedMs: (n) => `把查询转成向量花了 ${n} ms —— 这是到模型服务商的一次往返，不是 TiDB。每一列上的耗时只算 SQL。`,
    emptyTitle: "没有任何一行命中。",
  ...
  pt: {
    ...
    embedMs: (n) => `Gerar o embedding levou ${n} ms — uma ida e volta ao provedor do modelo, não ao TiDB. Os tempos por coluna são só do SQL.`,
  ...
  es: {
    ...
    embedMs: (n) => `Generar el embedding tardó ${n} ms — una ida y vuelta al proveedor del modelo, no a TiDB. Los tiempos por columna son solo del SQL.`,
